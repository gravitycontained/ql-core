...
namespace ql
{
	...
	namespace filesys
	{
		...
		ql::filesys::path& ql::filesys::path::operator=(const path& other)
		{
		...
		std::string ql::filesys::path::string() const
		{
		...
		std::string ql::filesys::path::read() const
		{
			...
			return ql::filesys::read_file(this->string());
		}
		...
		{
			ql::filesys::write_to_file(data, this->string());
		}
		...
		{
			std::wifstream file1(this->string(), std::wifstream::ate | std::wifstream::binary);
			std::wifstream file2(other.string(), std::wifstream::ate | std::wifstream::binary);
		...
		{
			ql::filesys::path copy = ql::to_string(this->get_parent_branch().string(), file_name, ".", this->get_file_extension());
			return copy;
		}

		ql::filesys::path ql::filesys::path::with_full_file_name(std::string file_name) const
		{
			ql::filesys::path copy = ql::to_string(this->get_parent_branch().string(), file_name);
			return copy;
		}

		ql::filesys::path ql::filesys::path::with_extension(std::string extension) const
		{
			ql::filesys::path copy = ql::to_string(this->get_parent_branch().string(), this->get_file_name(), ".", extension);
			return copy;
		}
		...
		void ql::filesys::path::remove() const
		{
			if (!this->exists())
			{
				ql::println("ql::filesys::path::remove: ", this->string(), " doesn't exist");
				return;
			}
		...
		void ql::filesys::path::copy(const ql::filesys::path& path_destination) const
		{
			if (!this->exists())
			{
				ql::println("ql::filesys::path::copy: ", this->string(), " doesn't exist");
				return;
			}
		...
		void ql::filesys::path::copy_overwrite(const ql::filesys::path& path_destination) const
		{
			if (!this->exists())
			{
				ql::println("ql::filesys::path::copy_overwrite: ", this->string(), " doesn't exist");
				return;
			}
			if (path_destination.is_directory())
			{
		...
		void ql::filesys::path::move(const ql::filesys::path& path_destination) const
		{
			if (!this->exists())
			{
				ql::println("ql::filesys::path::move: ", this->string(), " doesn't exist");
				return;
			}
		...
		void ql::filesys::path::move_overwrite(const ql::filesys::path& path_destination) const
		{
			if (!this->exists())
			{
				ql::println("ql::filesys::path::move_overwrite: ", this->string(), " doesn't exist");
				return;
			}
		...
		void ql::filesys::path::move_and_apply_path(const ql::filesys::path& path_destination)
		{
			if (!this->exists())
			{
				ql::println("ql::filesys::path::move_and_apply_path: ", this->string(), " doesn't exist");
				return;
			}
		...
		void ql::filesys::path::move_overwrite_and_apply_path(const ql::filesys::path& path_destination)
		{
			if (!this->exists())
			{
				ql::println("ql::filesys::path::move_overwrite_and_apply_path: ", this->string(), " doesn't exist");
				return;
			}
		...
		ql::filesys::paths ql::filesys::path::list_current_directory_tree() const
		{
			...
			auto str = this->is_directory() ? this->string() : this->get_parent_branch().string();
			for (auto i :
					 std::filesystem::recursive_directory_iterator(str, std::filesystem::directory_options::skip_permission_denied))
			{
				list.push_back({i.path().string()});
			}
			return {list};
		...
		void ql::filesys::path::update() const
		{
			...
			else
			{
				...
				this->m_is_file = this->m_is_directory ? false
																							 : std::filesystem::is_regular_file(this->string()) ||
																										 std::filesystem::is_block_file(this->string()) ||
																										 std::filesystem::is_character_file(this->string());
			}
		}
		...
		void ql::filesys::combine_files(const ql::filesys::paths& paths, const ql::filesys::path& destination)
		{
			...
			if (!file.good())
			{
				ql::println("there was a problem creating \"", destination.string(), "\"");
			}
			for (auto& i : paths)
			{
				file << i.read();
