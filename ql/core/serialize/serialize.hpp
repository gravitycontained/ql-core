...
namespace ql
{
	...
	namespace detail
	{
		...
		constexpr auto struct_member_size()
		{
			...
			{
				return struct_member_size<T, N + 1, true>();
			}
			else
			{
				return struct_member_size<T, N + 1, found>();
			}
		}
	}	 // namespace detail
	...
	constexpr auto struct_member_size()
	{
		return ql::detail::struct_member_size<T>();
	}

	namespace detail
	{
		template <class T, ql::size N>
		constexpr auto struct_to_tuple(T& n)
		{
			if constexpr (N == 0)
				return std::tuple<>{};
	...
	template <typename T>
	constexpr auto struct_to_tuple(T& value)
	{
