...
namespace ql
{
	...
	struct conditional_impl
	{
		using type = typename std::enable_if_t<
				(sizeof...(Args) == 0 && ql::is_same<Truth, ql::default_type>()) || ql::is_truth_type<Truth>(),
				std::conditional_t<
						(sizeof...(Args) >= 2),
						std::conditional_t<
								ql::is_true<Truth>(),
								std::conditional_t<ql::is_same<T, ql::error_type>(), T, ql::identity<T>>,
								ql::conditional_indirection<Args...>>,
						std::conditional_t<
								(sizeof...(Args) == 0),
								std::conditional_t<
										std::is_same_v<Truth, ql::default_type>,
										std::conditional_t<std::is_same_v<T, ql::error_type>, T, ql::identity<T>>,
