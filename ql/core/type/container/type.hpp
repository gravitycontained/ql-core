...
namespace ql
{
	...
	concept has_map_signature_c =
			(ql::is_container<C>() && ql::has_value_type<C>() && ql::has_mapped_type<C>() && ql::has_key_compare_type<C>() &&
	...
	constexpr bool is_std_set_type()
	{
		...
		{
			if constexpr (ql::has_key_type<C>() && ql::has_key_compare_type<C>() && ql::has_allocator_type<C>())
			{
				if constexpr (ql::is_same_decayed<
													C,
													std::set<ql::container_key_type<C>, ql::container_key_compare_type<C>, ql::container_allocator_type<C>>>(
											))
				{
					return true;
				}
	...
	constexpr bool is_std_multiset_type()
	{
		if constexpr (ql::is_container<C>())
		{
			if constexpr (ql::has_key_type<C>() && ql::has_key_compare_type<C>() && ql::has_allocator_type<C>())
			{
				if constexpr (ql::is_same_decayed<
													C, std::multiset<
																 ql::container_key_type<C>, ql::container_key_compare_type<C>, ql::container_allocator_type<C>>>(
											))
				{
					return true;
				}
	...
	constexpr bool is_std_unordered_set_type()
	{
		if constexpr (ql::is_container<C>())
		{
			if constexpr (ql::has_key_type<C>() && ql::has_hasher_type<C>() && ql::has_key_equal_type<C>() &&
										ql::has_allocator_type<C>())
			{
				if constexpr (ql::is_same_decayed<
													C, std::unordered_set<
																 ql::container_key_type<C>, ql::container_hasher_type<C>, ql::container_key_equal_type<C>,
	...
	constexpr bool is_std_map_type()
	{
		if constexpr (ql::is_container<C>())
		{
			if constexpr (ql::has_key_type<C>() && ql::has_mapped_type<C>() && ql::has_key_compare_type<C>() &&
										ql::has_allocator_type<C>())
			{
				if constexpr (ql::is_same_decayed<
													C, std::map<
																 ql::container_key_type<C>, ql::container_mapped_type<C>, ql::container_key_compare_type<C>,
																 ql::container_allocator_type<C>>>())
				{
					return true;
				}
	...
	constexpr bool is_std_unordered_map_type()
	{
		if constexpr (ql::is_container<C>())
		{
			if constexpr (ql::has_key_type<C>() && ql::has_mapped_type<C>() && ql::has_hasher_type<C>() &&
										ql::has_key_equal_type<C>() && ql::has_allocator_type<C>())
			{
				if constexpr (ql::is_same_decayed<
													C, std::unordered_map<
																 ql::container_key_type<C>, ql::container_mapped_type<C>, ql::container_hasher_type<C>,
																 ql::container_key_equal_type<C>, ql::container_allocator_type<C>>>())
				{
					return true;
				}
			}
	...
	constexpr bool is_std_vector_type()
	{
		if constexpr (ql::is_container<C>())
		{
			...
			{
				if constexpr (ql::is_same_decayed<C, std::vector<ql::container_value_type<C>, ql::container_allocator_type<C>>>())
				{
					return true;
				}
			}
	...
	constexpr bool is_std_list_type()
	{
		if constexpr (ql::is_container<C>())
		{
			if constexpr (ql::has_value_type<C>() && ql::has_allocator_type<C>())
			{
				if constexpr (ql::is_same_decayed<C, std::list<ql::container_value_type<C>, ql::container_allocator_type<C>>>())
				{
					return true;
				}
			}
	...
	constexpr bool is_std_forward_list_type()
	{
		if constexpr (ql::is_container<C>())
		{
			if constexpr (ql::has_value_type<C>() && ql::has_allocator_type<C>())
			{
				if constexpr (ql::is_same_decayed<C, std::forward_list<ql::container_value_type<C>, ql::container_allocator_type<C>>>())
				{
					return true;
				}
			}
	...
	constexpr bool is_std_deque_type()
	{
		if constexpr (ql::is_container<C>())
		{
			if constexpr (ql::has_value_type<C>() && ql::has_allocator_type<C>())
			{
				if constexpr (ql::is_same_decayed<C, std::deque<ql::container_value_type<C>, ql::container_allocator_type<C>>>())
				{
					return true;
				}
			}
	...
	constexpr bool is_std_array_type()
	{
		if constexpr (ql::is_container<C>())
		{
			...
			{
				if constexpr (ql::is_same<C, std::array<ql::container_value_type<C>, ql::std_array_size<C>()>>())
				{
					return true;
				}
			}
	...
	{
		return ql::is_std_set_type<C>() || ql::is_std_multiset_type<C>() || ql::is_std_map_type<C>() || ql::is_std_multimap_type<C>();
	}
	...
	{
		return ql::is_std_set_type<C>() || ql::is_std_multiset_type<C>() || ql::is_std_unordered_set_type<C>() ||
					 ql::is_std_unordered_multiset_type<C>() || ql::is_std_map_type<C>() || ql::is_std_multimap_type<C>() ||
					 ql::is_std_unordered_map_type<C>() || ql::is_std_unordered_multimap_type<C>() || ql::is_std_vector_type<C>() ||
					 ql::is_std_list_type<C>() || ql::is_std_forward_list_type<C>() || ql::is_std_array_type<C>() ||
	...
	template <typename T>
	concept has_square_brackets_read_c = requires(const T& n) { n[ql::detail::mapped_type<T>()]; };
	...
	template <typename T>
	concept has_square_brackets_write_c = requires(T& n) { n[ql::detail::mapped_type<T>()] = ql::detail::value_type<T>(); };
	...
	template <typename T>
	concept has_at_read_c = requires(const T& n) { n.at(ql::detail::mapped_type<T>()); };
	...
	template <typename T>
	concept has_at_write_c = requires(T& n) { n.at(ql::detail::mapped_type<T>()) = ql::detail::value_type<T>(); };
