...
namespace ql
{
	...
	constexpr bool is_long_string_type()
	{
		return ql::is_std_basic_string<T>() || ql::is_std_basic_string_view<T>() ||
					 ql::is_any_type_decayed_equal_to<T, const char*, char*, const char[], const wchar_t*, const wchar_t[], wchar_t*>();
	}

	template <typename T>
	constexpr bool is_long_standard_string_type()
	{
		return ql::is_any_type_decayed_equal_to<T, const char*, char*, const char[], std::string, std::string_view>();
	}
	...
	constexpr bool is_u32string_type()
	{
		return ql::is_any_type_decayed_equal_to<
				T, char32_t, const char32_t*, char32_t*, const char32_t[], std::basic_string<char32_t>, std::basic_string_view<char32_t>>(
		);
	}
	...
	{
		return ql::is_any_type_decayed_equal_to<T, char, wchar_t, char8_t, char16_t, char32_t>();
	}
