...
namespace ql
{
	...
	template <typename T>
	requires (ql::is_tuple<T>() && ql::lvalue<ql::tuple_type<ql::tuple_size<T>() - 1, T>>)
	...
	template <typename T>
	requires (ql::is_tuple<T>() && ql::rvalue<ql::tuple_type<ql::tuple_size<T>() - 1, T>>)
	constexpr auto&& tuple_value_back(T&& tuple)
	{
		return ql::tuple_value<ql::tuple_size<T>() - 1>(std::forward<T>(tuple));
	}
	...
	template <typename T>
	requires (ql::is_tuple<T>() && ql::no_reference<ql::tuple_type<ql::tuple_size<T>() - 1, T>>)
	constexpr auto tuple_value_back(T tuple)
	{
		return ql::tuple_value<ql::tuple_size<T>() - 1>(std::forward<T>(tuple));
	}
