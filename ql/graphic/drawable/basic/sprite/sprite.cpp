...
namespace ql
{
	...
	{
		sf::IntRect rect;
		rect.position.x = ql::i32_cast(hitbox.position.x);
		rect.position.y = ql::i32_cast(hitbox.position.y);
		rect.size.x = ql::i32_cast(hitbox.dimension.x);
		rect.size.y = ql::i32_cast(hitbox.dimension.y);
		this->m_sprite->setTextureRect(rect);
	}
	...
	{
		this->m_sprite = std::make_unique<sf::Sprite>(sprite);
		return *this;
	}
}
