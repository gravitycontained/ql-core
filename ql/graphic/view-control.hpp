...
namespace ql
{
	...
	struct view_control_t : ql::view_type<T>
	{
		...
		void move(T dx, T dy)
		{
			this->move(ql::vec(dx, dy));
		}

		void set_scale(ql::vector2<T> scale)
		{
			this->scale = scale;
			this->changed = true;
		}
		...
		void set_center(ql::vector2<T> position)
		{
		...
		void zoom_out(ql::vec2 mouse)
		{
			...
			if (this->use_smooth_zoom)
			{
				...
				this->animation_position_a = this->position;
				this->animation_position_b =
						position_start + (mouse_relative * (1.0 / (1 + this->zoom_delta) - 1)) * this->animation_scale_b;
