...
namespace ql
{
  ...
  struct vectorN : qpl::detail::vector_impl_conditional<T, N>
  {
    ...
    template <typename U> constexpr operator sf::Vector2<U>() const
    {
      if constexpr(N == 1)
        ...
        {
          return sf::Vector2<U>(static_cast<U>(this->data[0]),
  ...
  template <typename T, qpl::size N> struct matrixN
  {
    ...
    template <typename U>
    constexpr matrixN &
    operator=(const std::initializer_list<std::initializer_list<U>> &list)
    {
      if(list.size() == 0)
        {
          this->clear();
    ...
    constexpr matrixN rotated(A angle, const qpl::vector3<U> &vec) const
    {
      ...
      matrixN result;
      result[0] = this->data[0] * rotate[0].x + this->data[1] * rotate[0].y
                  + this->data[2] * rotate[0].z;
      result[1] = this->data[0] * rotate[1].x + this->data[1] * rotate[1].y
                  + this->data[2] * rotate[1].z;
      result[2] = this->data[0] * rotate[2].x + this->data[1] * rotate[2].y
                  + this->data[2] * rotate[2].z;
    ...
    template <typename U>
      requires(N == 4)
    [[nodiscard]] constexpr static matrixN
    perspective(U fovy, U aspect, U zNear, U zFar)
    {
    ...
    template <typename U>
      requires(N == 4)
    [[nodiscard]] constexpr static matrixN
    frustum(U left, U right, U bottom, U top, U nearVal, U farVal)
    {
      matrixN result(T{0});
  ...
  template <typename T> struct straight_line_t
  {
    ...
    maths_float_type length() const
    {
      ...
      return static_cast<maths_float_type>(
  ...
  template <typename T> struct hitbox_t
  {
    ...
    constexpr bool corners_collide_with(const qpl::hitbox_t<U> &hitbox) const
    {
      ...
      return hitbox.contains(p1) || hitbox.contains(p2) || hitbox.contains(p3)
             || hitbox.contains(p4);
    }
    ...
    bool collides(T x1, T x2, T y1, T y2, qpl::straight_line_t<U> line) const
    {
      ...
      auto x_cross = ((line.a.x < x1) && (line.b.x > x2))
                     || ((line.b.x < x1) && (line.a.x > x2));
      if(x_cross && (y_a_inside && y_b_inside))
        return true;

      auto y_cross = ((line.a.y < y1) && (line.b.y > y2))
                     || ((line.b.y < y1) && (line.a.y > y2));
