...
#include <ql/type-traits/arithmetic/bit-count-of-max-value.hpp>
...
namespace ql
{
	...
	using superior_floating_point = ql::conditional<ql::if_true<(ql::mantissa_bit_size<i2>() > ql::mantissa_bit_size<i1>())>,
																									i2,
	...
	namespace detail
	{
		...
		using superior_arithmetic_type =
				typename ql::conditional<ql::if_true<(ql::is_floating_point<T>() && ql::is_floating_point<U>())>,
		...
		constexpr auto superior_arithmetic_types()
		{
			return superior_arithmetic_types<decltype(superior_arithmetic_types<T1, T2>()),
			...
			ql::conditional<
					...
					ql::conditional<
							...
							ql::conditional<
									...
									ql::conditional<ql::if_true<(ql::bit_count_of_max_value<ql::superior_arithmetic_type<T, U>>() <
																							 ql::bit_count_of_max_value<ql::i64>())>,
